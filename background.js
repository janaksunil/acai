chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === "process_data") {
    ...
    const instruction = `Pretend you are a Chrome extension that detects if a user is using the wrong first name when sending an email. You have 3 inputs Receiver's email : ${email} , Owner ID (If applicable) :  ${ownerId},  Email body : ${text}". The email body should contain a phrase like Dear Name. Compare that Name with the receiver's email/Owner ID. Use your best judgment, if you think the name in the email body matches the receiver's email/name, return 1. if you think they don't match, return 0. Only alert the user if there is no possibility the first name is correct. Prioritize not disturbing the user. Respond only with 1 or 0, nothing else, no text`;
